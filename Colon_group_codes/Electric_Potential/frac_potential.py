...
Objective: To calculate electric potential along the grid from 0 to 2*Bondlength; so from Z = 0 Angstrom to
Z = 3.08 Angstorm 
"""
...
Z_del = 0.22
N_z = 2
Z_axis_probe = N_z*Z_del
Z = int(round(Z_axis_probe/Z_del))
...
dataset = pd.read_csv('IC_26_AA.csv',delimiter=',')
...
for x_counter in range(N+1):
	...
	for y_counter in range(N+1):
		for z_position in range(Z+1):
			...
			r_z[counter] = (z_position*Z_axis_probe/Z)
			#print(r_x[counter])
			counter += 1
...
for z_counter in range(Z+1):
	...
	z_filename = "F_AAZ"+str(z_counter)+".csv"
	d_z = (z_counter*Z_axis_probe/Z)
	#Creating a .csv file for storing data
